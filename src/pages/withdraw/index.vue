<template>
    <div class="point-topic">
        <div class="bg-top">
            <div class="total">
                <div class="total-exchange">
                    <div class="exchange-title">可提现</div>
                    <div class="exchange-value"> <span>{{ exchange }}</span>元</div>
                </div>
            </div>
            <div class="exchange">
        ...
        <div class="alipay-area">
            ...
            <div class="update-acc" @click="handlePayBundle">{{!alipayExist?'添加':'修改'}}</div>
        </div>
        <div class="point" v-load-more="loaderMore">
            <div class="point-title" v-if="drawList.length != 0">提现记录</div>
            <div class="point-item" v-for="item in drawList">
        ...
        <div class="pay-bundle-box-container" v-if="isPayBundleBoxShow">
            <div class="pay-bundle-box">
                ...
                <h4>{{!alipayExist?'请先添加支付宝账号':'修改支付宝账户'}}</h4>
                <p v-if="!alipayExist">只有添加完账号之后才能兑换积分，建议直接打开支付宝，直接复制账号哦</p>
        ...
        <div class="exchange-box-container" v-if="isExchangeBoxShow">
            <div class="exchange-box">
                ...
                <i class="icon icon-alipay"></i>
                <h4>请输入提现金额</h4>
                <input type="number" v-model="exchangeConfirm" ref="exchangeInput">
...
<script>
    ...
    export default {
        ...
        watch: {
            ...
            //     if (this.exchangeConfirm.split(".")[1] && [...this.exchangeConfirm.split(".")[1]].length > 2) {
            //         this.exchangeConfirm = this.exchangeConfirm.split(".")[0] + "." + this.exchangeConfirm.split(".")[1].substring(0, 2)
        ...
        methods: {
            ...
            getMoreExchangeList() {
                ...
                this.page++;
                _vue.$ajax.post(ApiControl.getApi(env, "drawList"), {
                    pageNo: this.page
                }).
                then(res => {
                    if (res.data.code == 0) {
                        ...
                        for (var i in res.data.result)
                            _vue.drawList.push(res.data.result[i]);
                        setTimeout(function() {
                            _vue.loading = false;
            ...
            validateNum() {
                ...
                document.onkeydown = function(e) {
                    if (e.keyCode == 110 || e.keyCode == 190 && _vue.isExchangeBoxShow) {
                        e.preventDefault();
                        return;
                    }
                }
            ...
            handleExchangeBoxCommit() {
                if (!this.exchangeConfirm || [...this.exchangeConfirm][0] == 0 && [...this.exchangeConfirm][1] != '.') {
                    this.exchangeBoxWarn = '请输入要提现的金额'
                    ...
                    //     this.isExchangeBoxWarn = true
                    // } else 
                    if (this.exchangeConfirm > this.exchange) {
                        this.exchangeBoxWarn = '输入金额只能不大于可提现金额'
                        this.$refs.exchangeInput.focus()
                        this.isExchangeBoxWarn = true
                    } else {
                        this.exchangeBoxWarn = ''
                        this.isExchangeBoxWarn = false
                        var _vue = this;
                        _vue.$ajax.post(ApiControl.getApi(env, "submitDraw"), {
                            // amount: parseInt(_vue.exchangeConfirm)
                            amount: parseFloat(_vue.exchangeConfirm)
                        }).
                        then(res => {
                            if (res.data.code == 0) {
        ...
        created() {
            ...
            _vue.$ajax.get(ApiControl.getApi(env, "drawList"), {}).
            then(res => {
                if (res.data.code == 0) {
                    _vue.drawList = res.data.result;
                } else {
                    // _vue.setErrorMessage(res.data.message);
                }
    ...
    .bg-top {
        ...
        .total {
            ...
            .total-exchange {
                ...
                margin: 0 auto;
                background: url('../../static/images/partner/partner-withdraw-circle.png') no-repeat center;
        ...
        .exchange {
            height: 40px;
            background: url('../../static/images/partner/partner-withdram-icon.png') no-repeat center center;
            background-size: contain;
            .exchange-button {
                height: 25px;
                width: 55px;
